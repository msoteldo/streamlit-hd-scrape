...
from publish_test import publish_or_update_products
import pandas as pd
import os
from blob import download_products, upload_products

st.title("Home Depot Product Info Tracker")

CSV_FILE = "products.csv"

# Descargar CSV del blob antes de trabajar
download_products(CSV_FILE)
...
try:
    ...
    "Stock Available": pd.Series(dtype="int"),
...
# Asegurar que Item_ID sea string
                ...
                try:
                    ...
                    for r in resultados:
                        ...
                        if error_msg and isinstance(error_msg, str) and error_msg.strip():
                            errores.append(f"SKU {sku_str}: {error_msg}")
                        else:
                            st.session_state.products_df.loc[
                                st.session_state.products_df["SKU"] == sku_str, "Item_ID"
                            ] = item_id
